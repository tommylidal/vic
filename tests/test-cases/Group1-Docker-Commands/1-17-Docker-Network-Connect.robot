...
Connect containers to multiple networks overlapping
    ...
    ${rc}  ${containerID}=  Run And Return Rc And Output  docker %{VCH-PARAMS} create --net cross1-network --name cross1-container2 busybox ping -c2 cross1-container
    Should Be Equal As Integers  ${rc}  0
    ${rc}  ${output}=  Run And Return Rc And Output  docker %{VCH-PARAMS} network connect cross1-network2 ${containerID}
    Should Be Equal As Integers  ${rc}  0
    ${rc}  ${output}=  Run And Return Rc And Output  docker %{VCH-PARAMS} start ${containerID}
...
Connect containers to multiple networks non-overlapping
    ...
    ${rc}  ${containerID}=  Run And Return Rc And Output  docker %{VCH-PARAMS} create --net cross2-network2 --name cross2-container2 busybox ping -c2 cross2-container
    Should Be Equal As Integers  ${rc}  0
    ${rc}  ${output}=  Run And Return Rc And Output  docker %{VCH-PARAMS} start ${containerID}
    Should Be Equal As Integers  ${rc}  0
...
#    Should Be Equal As Integers  ${rc}  0
#
#    ${rc}  ${containerID}=  Run And Return Rc And Output  docker %{VCH-PARAMS} create --net cross3-network --name cross3-container3 busybox /bin/sh -c "ping -c2 cross3-container && ping -c2 cross3-container2"
#    Should Be Equal As Integers  ${rc}  0
