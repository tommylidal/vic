...
*** Test Cases ***
Simple docker run
    ${rc}  ${output}=  Run And Return Rc And Output  docker %{VCH-PARAMS} run busybox dmesg
    Log  ${output}
    Should Be Equal As Integers  ${rc}  0

Docker run with -i
    ${rc}  ${output}=  Run And Return Rc And Output  docker %{VCH-PARAMS} run -i busybox dmesg
    Log  ${output}
    Should Be Equal As Integers  ${rc}  0

Docker run with -it
    ${rc}  ${output}=  Run And Return Rc And Output  mkfifo /tmp/fifo
    ${result}=  Start Process  docker %{VCH-PARAMS} run -i busybox /bin/top < /tmp/fifo  shell=True  alias=top
    Sleep  5
...
Simple docker run with app that doesn't exit
    ...
    #${result}=  Run Process  docker %{VCH-PARAMS} run busybox /bin/top  shell=True  timeout=5s  on_timeout=terminate
    #Log  ${result.stdout}
...
Docker run linked containers
    ${rc}  ${output}=  Run And Return Rc And Output  docker %{VCH-PARAMS} run --link busy3:busy3 busybox ping -c2 busy3
    Should Be Equal As Integers  ${rc}  0
...
Docker run df command
    ...
    #${rc}  ${output}=  Run And Return Rc And Output  docker %{VCH-PARAMS} run -it busybox /bin/df
    #Should Be Equal As Integers  ${rc}  0
...
Docker run date
    ...
    Run Keyword If  '${status}' == 'closed'  Fail  Test 1-6-Docker-Run.robot needs to be updated now that Issue #1578 has been resolved
    Log  Issue \#1578 is blocking implementation  WARN
    #${rc}  ${output}=  Run And Return Rc And Output  docker %{VCH-PARAMS} run busybox date
    #Should Be Equal As Integers  ${rc}  0
